...
class Complejo extends BaseController
{
    ...
    public function generateTable()
    {
        ...
        $complejo_list = $this->complejo_model->orderBy('nombre','ASC')->findAll();

        /**Generar tabla y botones */
        $table = new \CodeIgniter\View\Table([
            'table_open' => '<table id="tableComplejo" class="table table-striped mb-0 text-nowrap">'
        ]);
    ...
    public function store()
    {
        ...
        if ($this->complejo_model->save($data)) {
    ...
    public function edit($token = null)
    {
        ...
        $data['obj'] = $this->complejo_model->find($token);
        if ($data['obj'] == null) {
            return redirect()->to('/complejo');
        };
    ...
    {
        $this->complejo_model->delete($id);
        return 'Médico eliminado.';
    }
    public function changeStatus($id)
    {
        $estado = $this->request->getPost('estado');
        echo ($this->complejo_model->save(['id_complejo' => $id, 'valido' => $estado==0?false:1]));
    }
}
